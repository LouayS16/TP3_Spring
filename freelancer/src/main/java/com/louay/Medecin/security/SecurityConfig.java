package com.louay.Medecin.security;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter{
	...
	auth.inMemoryAuthentication().withUser("louay").password(passwordEncoder.encode("123")).roles("AGENT","USER");
	auth.inMemoryAuthentication().withUser("user1").password(passwordEncoder.encode("123")).roles("USER");
	 ...
	 protected void configure(HttpSecurity http) throws Exception {
		 ...
		 .antMatchers("/supprimerMedecin","/modifierMedecin","/updateMedecin")
		 .hasAnyRole("ADMIN");
